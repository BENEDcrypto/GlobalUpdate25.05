...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, isAsync) {
        ...
        //convert BENED to NQT...
        var field = "N/A";
        try {
            var xFields = [
                ["feeBENED", "feeNQT"],
                ["amountBENED", "amountNQT"],
                ["priceBENED", "priceNQT"],
                ["refundBENED", "refundNQT"],
                ["discountBENED", "discountNQT"],
                ["phasingQuorumBENED", "phasingQuorum"],
                ["phasingMinBalanceBENED", "phasingMinBalance"],
                ["controlQuorumBENED", "controlQuorum"],
                ["controlMinBalanceBENED", "controlMinBalance"],
                ["controlMaxFeesBENED", "controlMaxFees"],
                ["minBalanceBENED", "minBalance"],
                ["shufflingAmountBENED", "amount"],
                ["monitorAmountBENED", "amount"],
                ["monitorThresholdBENED", "threshold"]
            ];

            for (i = 0; i < xFields.length; i++) {
                var xField = xFields[i][0];
                var nqtField = xFields[i][1];
                if (xField in data) {
                    data[nqtField] = NRS.convertToNQT(data[xField]);
                    delete data[xField];
                }
            }
        } catch (err) {
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToBENED(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var bndAddress = new BenedAddress();
            var accountRS = "";
            if (bndAddress.set(accountId)) {
                accountRS = bndAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, isAsync) {
        ...
        var url = NRS.server + "/xsrv?requestType=" + requestType;
    ...
    NRS.broadcastTransactionBytes = function (transactionData, callback, originalResponse, originalData) {
        $.ajax({
            url: NRS.server + "/xsrv?requestType=broadcastTransaction",
            crossDomain: true,
            dataType: "json",
            type: "POST",
            timeout: 30000,
        ...
        NRS.sendRequest("encodeQRCode",
            ...
            function(response) {
                if('qrCodeBase64' in response) {
                    $(target).empty().append(
                        $("<img src='data:image/jpeg;base64,"+response.qrCodeBase64+"'>"+NRS.createCopyButton("https://resurc_dla_qrcode/?to=" + escape(qrCodeData)))
                    );
                }
            },
            true
    ...
    function addAddressData(data) {
        if (typeof data == "object" && ("recipient" in data)) {
            ...
            if (/^bened/i.test(data.recipient)) {
