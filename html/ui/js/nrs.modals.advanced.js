...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumBENED",
			addonText: "BENED",
			addonI18n: "cointech_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_cointech', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceBENED';
		context['addonText'] = 'BENED';
		context['addonI18n'] = 'cointech_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_cointech', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
