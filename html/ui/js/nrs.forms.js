...
var NRS = (function(NRS, $) {
    ...
    NRS.submitForm = function($modal, $btn) {
		...
		$form.find(":input").each(function() {
			if ($(this).is(":invalid")) {
				var error = "";
				var name = String($(this).attr("name")).replace("BENED", "").replace("NQT", "").capitalize();
				var value = $(this).val();
		...
		if (!data) {
        	...
        	data.feeBENED = data.amountBENED*0.01 < 0.150000 ? 0.150000 : ((data.amountBENED*0.01) < 100 ? (data.amountBENED*0.01) : 100 );
		data.feeBENED = isNaN(data.feeBENED) ? 0.150000 : data.feeBENED;
		...
		if (!NRS.showedFormWarning) {
			if ("amountBENED" in data && NRS.settings["amount_warning"] && NRS.settings["amount_warning"] != "0") {

				try {
					var amountNQT = NRS.convertToNQT(data.amountBENED);
				} catch (err) {
				...
				if (new BigInteger(amountNQT).compareTo(new BigInteger(NRS.settings["amount_warning"])) > 0) {
					...
					$form.find(".error_message").html($.t("error_max_amount_warning", {
						"techcoin": NRS.formatAmount(NRS.settings["amount_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			if ("feeBENED" in data && NRS.settings["fee_warning"] && NRS.settings["fee_warning"] != "0") {

				try {

					var feeNQT = NRS.convertToNQT(data.feeBENED);
				...
				if (new BigInteger(feeNQT).compareTo(new BigInteger(NRS.settings["fee_warning"])) > 0) {
					...
					$form.find(".error_message").html($.t("error_max_fee_warning", {
						"techcoin": NRS.formatAmount(NRS.settings["fee_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			var convertBENEDFields = ["phasingQuorumBENED", "phasingMinBalanceBENED"];
			$.each(convertBENEDFields, function(key, field) {
				if (field in data) {
    ...
    function updateFee(modal, feeNQT) {
        ...
        fee.val(NRS.convertToBENED(feeNQT));
        var recalcIndicator = $("#" + modal.attr('id').replace('_modal', '') + "_recalc");
