...
var NRS = (function(NRS, $) {
	...
	NRS.pages.settings = function() {
		...
		for (var key in NRS.settings) {
            ...
            if (/_warning/i.test(key) && key != "asset_transfer_warning" && key != "currency_transfer_warning") {
				if (setting.length) {
					setting.val(NRS.convertToBENED(NRS.settings[key]));
				}
	...
	NRS.createLangSelect = function() {
		...
		var $langSelBoxesTop = $('.choice-language-top .dropdown-menu')
		$langSelBoxes.empty();
		$langSelBoxesTop.empty();
		$.each(NRS.languages, function(code, name) {
			$langSelBoxes.append('<option value="' + code + '">' + name + '</option>');
			$langSelBoxesTop.append('<li><a data-value="' + code + '">' + name + '</a></li>');
		});
	...
	$('.choice-language-top').on('click', '.dropdown-menu li a', function(e){
		e.preventDefault();
		NRS.updateSettings('language', $(this).attr("data-value"));
	});
