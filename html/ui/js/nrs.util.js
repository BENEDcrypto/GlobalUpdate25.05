...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.calculateOrderPricePerWholeQNT = function (price, decimals, returnAsObject) {
		...
		return NRS.convertToBENED(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	};
    ...
    NRS.calculateOrderTotal = function (quantityQNT, priceNQT) {
        return NRS.convertToBENED(calculateOrderTotalImpl(quantityQNT, priceNQT));
    };
    ...
    NRS.convertToBENED = function (amount, returnAsObject) {
        var negative = "";
        var mantissa = "";
        ...
        var fractionalPart = amount.mod(new BigInteger("1000000")).toString();
        amount = amount.divide(new BigInteger("1000000"));

        if (fractionalPart && fractionalPart != "0") {
            mantissa = ".";

            for (var i = fractionalPart.length; i < 6; i++) {
                mantissa += "0";
            }
    ...
    NRS.convertToNQT = function (currency) {
		...
		if (parts.length == 1) {
			...
			if (parts[1].length <= 6) {
                ...
                fraction = parts[1].substring(0, 6);
			}
		} else {
    ...
    NRS.formatAmount = function (amount, round, no_escaping, zeroPad) {
        ...
        if (typeof amount == "object") {
            var params = NRS.convertToBENED(amount, true);
        ...
        if (NRS.settings) {
            ...
            var maxLength = parseInt(NRS.settings.max_cointech_decimals) + offset;
    ...
    NRS.dataItemsLoaded = function (data, noPageLoad) {
        $el = $("#transactions_items");
        $el.find(".items").empty().append(data);
        $el.find('[data-toggle="tooltip"]').tooltip();

        $el2 = $("#transactions_items_dashboard");
        $el2.find(".items.transactions").empty().append(data);
        $el2.find('[data-toggle="tooltip"]').tooltip();

		NRS.dataLoadFinished($el);
        NRS.dataLoadFinished($el2);
    ...
    NRS.dataDashLoadedBlocks = function (data, noPageLoad) {
        $el2 = $("#transactions_items_dashboard");
        $el2.find(".items.messages").empty().append(data);
        $el2.find('[data-toggle="tooltip"]').tooltip();
  ...
  NRS.dataLoadFinished = function ($el, fadeIn) {
    ...
    NRS.createInfoTable = function (data, fixed) {
		...
		for (var key in data) {
            ...
            rows += "<tr><td class='test' style='font-weight:bold" + (fixed ? ";width:150px" : "") + "'>" + $.t(key).escapeHTML() + (type ? " " + type.escapeHTML() : "") + ":</td><td style='width:80%;text-transform:none;word-break:break-all'>" + value + "</td></tr>";
        }
