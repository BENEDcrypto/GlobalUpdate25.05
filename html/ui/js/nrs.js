/******************************************************************************
 * Copyright © 2020-2021 The SODLO uno Developers.                             *
 *                                                                            *
...
//				NRS.checkAliasVersions();
    ...
    NRS.darkBened = function () {
        if (s.length > 0) {
            console.log("Enabled day theme");
            s.remove();
        } else {
            console.log("Enabled night theme");
            $('head').append('<link rel="stylesheet" type="text/css" id="dark" href="css/dark.css">');
        }
    }
    ...
    NRS.getState = function (callback, msg) {
        ...
        NRS.sendRequest("getBlockchainStatus", {}, function (response) {
            if (response.errorCode) {
                ...
                $('#dashboard-amount-blocks').html(NRS.state.numberOfBlocks);
                ...
                if (NRS.account && !_firstTimeAfterLoginRun) {
                    ...
                    $('#dashboard-blocks-lastdate').html(NRS.formatTimestamp(NRS.blocks[0].timestamp));
                    $('#dashboard-blocks-lastheight, .progress-bar .height .value').html("<a href='#' class='block show_block_modal_action' data-blockid='"+ NRS.state.lastBlock +"'>" + NRS.blocks[0].height + "</a>");
                }
...
//				NRS.setClosedGroups(result[0].contents.split("#"));
 ...
 // __YA__ conected_indicator is softmining or not pir conect
            ...
            if (response.peers && response.peers.length) {
              ...
              //  connectedIndicator.addClass("green");
              ...
              //  connectedIndicator.removeClass("green");
    ...
    NRS.getAccountInfo = function (firstRun, callback, isAccountSwitch) {
        NRS.sendRequest("getAccount", {
            ...
            if (response.errorCode) {
                ...
                var _softMG;
                var emission = 15000000000;


                NRS.sendRequest("getBlockchainStatus", {}, function (status) {
                    if (status != null && status.numberOfBlocks != null && status.numberOfBlocks > 10) { //571800) {
//genesis adres
                        NRS.sendRequest("getAccount+", {"account": NRS.account}, function (response) {
                            if (response != null && response.balanceNQT != null) {
                                ...
                                NRS.sendRequest("getSoftMGs+", {"account": NRS.account}, function (response) {
                                    if (response.balance != null) {
                                        var forgePercent = response._forgePercent;
                                        
                                    


                                        _softMG = response;
                                        var multi = Number(_softMG.multiplier);
                                        if (multi < 0.0012)
                                        ...
                                        NRS.softMGCounterId = setInterval(function () {
                                            ...
                                            if (_forPerc > 0) {
                                                ...
                                                connectedIndicator.removeClass("green");
                                                connectedIndicator.addClass("red");
                                                //connectedIndicator.find("span").html($.t("Not Connected")).attr("data-i18n", "not_connected");
                                                connectedIndicator.show();
                                    }    
                                        }, 500);
        ...
        if (NRS.accountInfo.accountControls && $.inArray('PHASING_ONLY', NRS.accountInfo.accountControls) > -1) {
            NRS.sendRequest("getPhasingOnlyControl", {
                ...
                if (response && response.votingModel >= 0) {
                    ...
                    if (response.maxFees) {
                        data.maximum_fees = NRS.convertToBENED(response.maxFees);
                    }
