 ...
 *
 *   java -cp "classes:lib/*:conf" bened.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dbened.runtime.mode=desktop bened.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "bened.testDb" : "bened.db";
        String dbType = Bened.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Bened.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Bened.getDbDir(Bened.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Bened.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Bened.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Bened.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "bened.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "bened.mv.db");
            if (!dbFile.exists()) {
        ...
        try {
            ...
            switch (phase) {
                ...
                case 1:
                    ...
                    //
                    File newFile = new File(dbDir, "bened.h2.db");
                    if (newFile.exists()) {
                        if (!newFile.delete()) {
                            Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                        }
                    } else {
                        newFile = new File(dbDir, "bened.mv.db");
                        if (newFile.exists()) {
                            if (!newFile.delete()) {
                                Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                            }
