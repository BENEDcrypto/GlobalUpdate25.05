...
import bened.util.CountingInputReader;
import bened.util.CountingInputStream;
import bened.util.CountingOutputWriter;
...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (Bened.APPLICATION.equals(application)) {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Bened.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = Bened.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_BND) / Constants.START_EMISION_BND);
    }
    ...
    public void blacklist(String cause) {
        blacklistingTime = Bened.getEpochTime();
    ...
    void connect() {
        lastConnectAttempt = Bened.getEpochTime();
        try {
    ...
    boolean analyzeHallmark(final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.START_EMISION_BND * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
