...
public final class DebugTrace {
    ...
    static final String QUOTE = Bened.getStringProperty("bened.debugTraceQuote", "\"");
    static final String SEPARATOR = Bened.getStringProperty("bened.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = Bened.getBooleanProperty("bened.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Bened.getStringListProperty("bened.debugTraceAccounts");
        String logName = Bened.getStringProperty("bened.debugTraceLog");
        ...
        Bened.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Bened.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Bened.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Bened.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
        return debugTrace;
    }
   ...
   private void traceBeforeAccept(Block block) {
    ...
    private Map<String, String> getValues(long accountId, boolean unconfirmed) {
        ...
        map.put("timestamp", String.valueOf(Bened.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Bened.getBlockchain().getHeight()));
