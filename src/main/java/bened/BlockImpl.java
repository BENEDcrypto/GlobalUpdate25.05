...
final class BlockImpl implements Block {
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength, byte[] payloadHash,
        ...
        this.generationSignature = generationSignature ; //  generationSignature;  // kastyl pri perehode na 2.1.214
    ...
    byte[] bytes() {
        ...
        if (bytes == null) {                                                                                          //dolzno bit 64 a tut 32  
            ByteBuffer buffer = ByteBuffer.allocate(4 + 4 + 8 + 4 + (version < 3 ? (4 + 4) : (8 + 8)) + 4 + 32 + 32 + 32 +(version>1?32:0) + (blockSignature!=null?64:0));
...
private static final long[] badBlocks = new long[]{};
    ...
    private void calculateBaseTarget(BlockImpl previousBlock) {
        ...
        BigInteger CUMULATIVE_DIFFICULTY_MULTIPLIER = Convert.two64.multiply(BigInteger.valueOf(Constants.BLOCK_TIME ));
        ...
        int targetBlocktime =  Constants.BLOCK_TIME/ Constants.USKORITEL_GENERACII_BLOCKOV;
        int blocktimeAverage = (this.timestamp- previousBlock.timestamp); // - block.timestamp) / 3;
        if (blocktimeAverage > targetBlocktime) {
            baseTarget = (prevBaseTarget * Math.min(blocktimeAverage, targetBlocktime + Constants.BLOCKTIME_DELTA)) / targetBlocktime;
        } else {
            baseTarget = prevBaseTarget - prevBaseTarget * Constants.BLOCKTIME_DELTA* (targetBlocktime - Math.max(blocktimeAverage, targetBlocktime - Constants.BLOCKTIME_DELTA)) / (100 * targetBlocktime);
        }
