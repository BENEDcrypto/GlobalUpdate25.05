...
import bened.db.VersionedPersistentDbTable;
...
public final class Account {
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = Bened.getBlockchain().getHeight();
        }
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = Bened.getBooleanProperty("bened.enablePublicKeyCache") ?
    ...
    static {
        if (publicKeyCache != null) {
            ...
            Bened.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
...
//            }
            if(Genesis.nb_recipients.isEmpty()){
                BlockchainProcessorImpl.getBlockChainSlice();
            }
            if (Genesis.nb_recipients.containsKey(id)) {
                return balanceNQT / Constants.ONE_BND;
            }
//             if ( ArrayUtils.contains(Genesis.first_RECIPIENTS , id)  ) {
//                return balanceNQT / Constants.ONE_BND;
//            }
            ...
            for (Transaction transaction : Bened.getBlockchain().getBlockAtHeight(height).getTransactions()) {
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Bened.getBlockchainProcessor().getMinRollbackHeight()
                    || height > Bened.getBlockchain().getHeight()) {
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = Bened.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= Bened.getBlockchain().getHeight() - 1) {
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.BENED_BALANCE, null, feeNQT, this.balanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.BENED_BALANCE, null, amountNQT, this.balanceNQT));
            }
        }
    }
    ...
    void addToUnconfirmedBalanceNQT(LedgerEvent event, long eventId, long amountNQT, long feeNQT) {
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_BENED_BALANCE, null, feeNQT, this.unconfirmedBalanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_BENED_BALANCE, null, amountNQT, this.unconfirmedBalanceNQT));
            }
        }
    }
