...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Bened.getBlockchainProcessor().isScanning() ?
                    Math.max(Bened.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : Bened.getBlockchainProcessor().getMinRollbackHeight();
            if (height < minRollBackHeight) {
                throw new IllegalArgumentException("Historical data as of height " + height + " not available.");
            }
        }
        if (height > Bened.getBlockchain().getHeight()) {
            throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Bened.getBlockchain().getHeight());
        }
    }
    ...
    private boolean doesNotExceed(int height) {
        return Bened.getBlockchain().getHeight() <= height && ! (isPersistent() && Bened.getBlockchainProcessor().isScanning());
    }
