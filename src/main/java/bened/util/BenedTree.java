...
import static bened.http.JSONResponses.ERROR_NOT_ALLOWED;

public class BenedTree {

    public static final boolean     ALLOW_API_HIERARCHY_WITHOUT_PASSWORD =  Bened.getBooleanProperty("bened.allowAPIHierarchyWithoutPassword", false);
    public static final boolean     ALLOW_API_HIERARCHY_ONLY_LOCALHOST   =  Bened.getBooleanProperty("bened.allowAPIHierarchyOnlyLocalhost", true);
    ...
    public static AccountMinimal getParentOf (long accountId) {
        ...
        if (parentID == 0L)
            return null;
        try {
            try (PreparedStatement statement = Bened.softMG().getConnection().prepareStatement("select balance,amount from soft where id=?")) {
    ...
    public static List<AccountMinimal> getDirectChildrenOf (long accountId, int internalParentID, int internalIDStart, boolean limit, int startIndex, boolean orderByAmount) throws SQLException {
        ...
        try {
            try (PreparedStatement statement = Bened.softMG().getConnection().prepareStatement("select id,balance,amount from soft where parent_id=?"  + (orderByAmount?" order by amount desc":"") + (limit?" limit 100"+(startIndex>0?" offset ?":""):"") )) {
                statement.setLong(1, accountId);
                    ...
                    while (rs.next()) {
                        ...
                        try {
                            child = new AccountMinimal(rs.getLong(1), internalIDStart++, internalParentID, rs.getLong(2), rs.getLong(3));
                             if (child.reedSolomonMinimal().equalsIgnoreCase("5H5H-PRS4-UYTA-8GY83")) 
                            continue;
                        } catch (Exception e) {
                            Logger.logErrorMessage(e.getMessage(), e);
                            continue;
    ...
    public static int getChildCountOf (long accountId) {
        try {
            try (PreparedStatement statement = Bened.softMG().getConnection().prepareStatement("select count(*) from soft where parent_id=?")) {
                statement.setLong(1, accountId);
                try (ResultSet rs = statement.executeQuery()) {
                    while (rs.next()) {
    ...
    public static class AccountMinimal {
        ...
        public AccountMinimal (long id, int internalID, int parentInternalID, long balance, long amount) throws Exception {
            ...
            if (a == null && id != 0L)  throw new Exception("Invalid account: " + id);
            this.id = id;
            this.internalID = internalID;
            this.parentInternalID = parentInternalID;
            this.balance = balance;
            this.amount = amount;
            this.forged = Bened.getBlockchain().getBlockCount(id);
            if (id != 0L) {
