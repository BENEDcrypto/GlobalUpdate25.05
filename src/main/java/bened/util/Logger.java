...
/**
 * Handle logging for the Bened node server
 */
public final class Logger {
     ...
     * The bened/conf/logging-default.properties and bened/conf/logging.properties configuration
    ...
    static {
        ...
        if (!Boolean.getBoolean("bened.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Bened.loadProperties(loggingProperties, "logging-default.properties", true);
                Bened.loadProperties(loggingProperties, "logging.properties", false);
                Bened.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(bened.Bened.class);
        enableStackTraces = Bened.getBooleanProperty("bened.enableStackTraces");
        enableLogTraceback = Bened.getBooleanProperty("bened.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof BenedLogManager) {
            ((BenedLogManager) LogManager.getLogManager()).coreShutdown();
        }
    }
