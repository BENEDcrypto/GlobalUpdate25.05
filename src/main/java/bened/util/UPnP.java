...
public class UPnP {
    ...
    public static synchronized void addPort(int port) {
        ...
        try {
            if (gateway.addPortMapping(port, port, localAddress.getHostAddress(), "TCP",
                                       Bened.APPLICATION + " " + Bened.VERSION)) {
    ...
    private static void init() {
        ...
        try {
            ...
            GatewayDevice.setHttpReadTimeout(Bened.getIntProperty("bened.upnpGatewayTimeout", GatewayDevice.getHttpReadTimeout()));
            GatewayDiscover discover = new GatewayDiscover();
            discover.setTimeout(Bened.getIntProperty("bened.upnpDiscoverTimeout", discover.getTimeout()));
