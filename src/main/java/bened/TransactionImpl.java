...
public final class TransactionImpl implements Transaction {
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws BNDException.NotValidException {
        ...
        if ( builder.feeNQT <= 0 || (Constants.correctInvalidFees && builder.signature == null) ) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Bened.getBlockchain().getHeight());
        ...
        if (bytes == null) {
            try {
                ...
                if (useNQT()) {
                    ...
                    buffer.putInt((int) (amountNQT / Constants.ONE_BND));
                    buffer.putInt((int) (feeNQT / Constants.ONE_BND));
    ...
    private boolean useNQT() {
        return this.height > Constants.NQT_BLOCK
                ...
                || Bened.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
...
//        }
    ...
    public void validate() throws BNDException.ValidationException {
        ...
        if ((timestamp == 0 ? (deadline != 0 || feeNQT != 0) : (deadline < 1 || feeNQT < 0))
                ...
                || feeNQT > Constants.START_BALANCE_centesimo
                || amountNQT < 0
                || amountNQT > Constants.START_BALANCE_centesimo
                || type == null) {
