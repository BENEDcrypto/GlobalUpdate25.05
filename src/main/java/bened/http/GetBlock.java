...
import static bened.http.JSONResponses.INCORRECT_BLOCK;
import static bened.http.JSONResponses.INCORRECT_HEIGHT;
import static bened.http.JSONResponses.INCORRECT_TIMESTAMP;
import static bened.http.JSONResponses.UNKNOWN_BLOCK;
...
public final class GetBlock extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (blockValue != null) {
            try {
                blockData = Bened.getBlockchain().getBlock(Convert.parseUnsignedLong(blockValue));
            ...
            try {
                ...
                blockData = Bened.getBlockchain().getBlockAtHeight(height);
            } catch (RuntimeException e) {
                return INCORRECT_HEIGHT;
            }
            ...
            try {
                ...
                blockData = Bened.getBlockchain().getLastBlock(timestamp);
            } catch (RuntimeException e) {
                return INCORRECT_TIMESTAMP;
            }
        } else {
            blockData = Bened.getBlockchain().getLastBlock();
        }
