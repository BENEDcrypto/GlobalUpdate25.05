...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Bened.getBooleanProperty("bened.enableAPIProxy") && ! API.isOpenAPI);
    private static final int blacklistingPeriod = Bened.getIntProperty("bened.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Bened.getStringProperty("bened.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Bened.getBlockchainProcessor().isDownloading());
    }
    ...
    boolean blacklistHost(String host) {
        ...
        blacklistedPeers.put(host, Bened.getEpochTime() + blacklistingPeriod);
