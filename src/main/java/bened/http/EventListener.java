...
import bened.TransactionProcessor;
 ...
 * has not been received within bened.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by bened.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Bened.getIntProperty("bened.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Bened.getIntProperty("bened.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Bened.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Bened.getTransactionProcessor();
    ...
    /** Bened event listeners */
    private final List<BenedEventListener> BenedEventListeners = new ArrayList<>();
 ...
 Bened event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Bened event registrations
     ...
     *
     * @param   eventRegistrations      Bened event registrations
     * @throws  EventListenerException  Invalid Bened event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<BenedEventListener> it = BenedEventListeners.iterator();
                while (it.hasNext()) {
                    BenedEventListener listener = it.next();
                ...
                if (addListener) {
                    BenedEventListener listener = new BenedEventListener(event);
                    listener.addListener();
                    BenedEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void removeEvents(List<EventRegistration> eventRegistrations) {
        ...
        try {
            ...
            //
            if (BenedEventListeners.isEmpty())
                deactivateListener();
        } finally {
            lock.unlock();
        }
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            BenedEventListeners.forEach(BenedEventListener::removeListener);
        } finally {
            lock.unlock();
        }
    ...
    /**
     * Bened event listener
     */
    private class BenedEventListener {

        /** Event handler */
        private final BenedEventHandler eventHandler;

        /**
         * Create the Bened event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public BenedEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Bened listener for this event
         */
        ...
        /**
         * Remove the Bened listener for this event
         */
        ...
        /**
         * Return the hash code for this Bened event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Bened events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof BenedEventListener) &&
                    eventHandler.equals(((BenedEventListener)obj).eventHandler));
        }

        /**
         * Bened listener event handler
         */
        private abstract class BenedEventHandler {
            ...
            /** Bened listener event */
            protected final Enum<? extends Enum> event;

            /**
             * Create the Bened event handler
             *
             * @param   eventRegistration   Event registration
             */
            public BenedEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof BenedEventHandler) &&
                                        owner == ((BenedEventHandler)obj).owner &&
                                        accountId == ((BenedEventHandler)obj).accountId &&
                                        event == ((BenedEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends BenedEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends BenedEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends BenedEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends BenedEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Bened listener event
        ...
        /**
         * Return the Bened listener event
         *
         * @return                  Bened listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
